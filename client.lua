...
local konum2 = vector3(283.2527, -978.5802, 29.43152)
local konum3 = vector3(287.0769, -978,      29.43152)

-- [FIX 2] Module-level handle to the active customer ped.
-- Exposed here so the onResourceStop handler can reach it regardless of
-- which state the inner pedspawn thread is in when the resource stops.
local activePed = nil
...
AddEventHandler('onResourceStop', function(resourceName)
    ...
    if activePed and DoesEntityExist(activePed) then
        ...
        SetPedAsNoLongerNeeded(activePed)
...
function siparisOlustur()
    ...
    if pizzaboyu == 1 then
        ...
        suankiSiparis.pizzaucret = math.random(1, 10)
    elseif pizzaboyu == 2 then
        suankiSiparis.pizza      = "pizzaorta"
        suankiSiparis.pizzaucret = math.random(10, 20)
    else
        suankiSiparis.pizza      = "pizzabuyuk"
        suankiSiparis.pizzaucret = math.random(20, 30)
    end
end

Citizen.CreateThread(function()
    while true do
        local sleepthread = 2000            -- [FIX 5] local, not global
...
function pedspawn()
    ...
    local n       = -10
    local o       = 0
    local b               -- intentionally nil; guarded before DeleteEntity [FIX 4]
    local p               -- intentionally nil; guarded before DeleteEntity [FIX 4]
    local q       = 200
    ...
    Citizen.CreateThread(function()
        ...
        if u == 1 then m = -35 else m = 35 end
        if v == 1 then n = -35 else n = 35 end

        Wait(500)
        local w, A = GetGroundZFor_3dCoord(t.x + m, t.y + n, t.z, 0)

        l = CreatePed(26, r, 281.5121, -934.8923, 29.2124, 28, 268.9422, true, true)
        activePed = l   -- [FIX 2] expose to the resource-stop cleanup handler
        ...
        local peddevam = true   -- [FIX 6] was an implicit global; now local

        while Bs do
            Citizen.Wait(500)
            local C  = GetEntityCoords(l)
            local t2 = GetEntityCoords(GetPlayerPed(-1))
            q        = GetEntityHealth(l)
            local D  = Vdist(C.x, C.y, C.z, t2.x, t2.y, t2.z)
            o        = o + 1
        ...
        if o >= 75 or q < 1.0 then
            ...
            activePed      = nil    -- [FIX 2] clear so stop handler skips it
            siparis_talebi = false
            teslim         = false
        else
            siparis_talebi = false
            teslim         = false
            suankiSiparis  = {}     -- [FIX 7] clear stale order data after delivery

            Wait(1550)

            -- [FIX 4] b and p are nil when never assigned; guard before calling
            -- DeleteEntity to avoid silently targeting entity 0 (local player ped).
            if b and DoesEntityExist(b) then DeleteEntity(b) end
            if p and DoesEntityExist(p) then DeleteEntity(p) end

            local pid = PlayerPedId()   -- [FIX 3] was undefined; resolve at call-site
            ClearPedTasks(pid)
            ClearPedTasks(l)
            TaskWanderStandard(l, 10.0, 10)
            SetPedAsNoLongerNeeded(l)
            activePed = nil             -- [FIX 2] clear cleanup reference

            -- [FIX 6] These were implicit globals written and never read again.
            -- Declaring them local here scopes them to this block only.
    ...
    function menuTeslim()
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ventacyc', {
            ...
            elseif data.current.value == "teslimet" then
                menu.close()
                TriggerServerEvent("s4:pizza:server:teslimat", suankiSiparis)
                Bs     = false
                teslim = false

            elseif data.current.value == "iptal" then
                menu.close()
                teslim = false
            end
